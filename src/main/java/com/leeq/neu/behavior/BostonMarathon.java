...
//  }
  ...
  ALoR insertByName(Runner r) {
    if (r.byName(this.first)) return new ConsLoR(r, this);
    else return new ConsLoR(this.first, this.rest.insertByName(r));
  }
  
  // insert the given runner (by name) into this sorted list of runners
  ALoR insertByAge(Runner r) {
    if (r.byAge(this.first)) return new ConsLoR(r, this);
    else return new ConsLoR(this.first, this.rest.insertByAge(r));
  }
...
class ExamplesMarathon {
  ...
  boolean testSortNames1(Tester t) {
    return t.checkExpect(this.list2.sortByName(), new ConsLoR(joan, new ConsLoR(johnny, new ConsLoR(bill, new ConsLoR(frank, mtlist)))));
  }
